...
class EditForm extends React.Component {
    ...
    handleSubmitUpdate(data) {
        ...
        model.add(data, function(response) {
            if (response && response.data) {
                console.log(data);
                router.push({pathname:'/xpage/list',query:{...that.props.query}});
            }
        })
    ...
    handleSaveAndEdit(childModuleName,data) {
        ...
        model.add(data, function(response) {
                if (response && response.data) {
                    ...
                    let params = {...that.props.query,xpageId:response.data.id,fromModule:'xpage'};
    ...
    handleAssociationEdit(associationModule,referm,data) {
        ...
        model.add(data, function(response) {
            if (response && response.data) {
                console.log(response.data);
                let params = {...that.props.query,moduleName:"xpage",moduleId:response.data.id,associationName:associationModule,referModule:referm};
...
render()
{
    ...
    return (
            ...
            <Form  onSubmit={this.handleSubmit.bind(this)}>
                ...
                <Form.Item >
                    <XList  onEdit ={that.onAssociationEdit.bind(that,'xpagepageinterface','')} refer ="xpagepageinterface" mapField="xpageId" byId='-1'  title="用到公用接口" />
                </Form.Item>

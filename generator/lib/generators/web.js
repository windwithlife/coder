...
var config = {
    ...
    templatePath: function(){return this.workTemplateRootPath()  + this.sideName + "/" + "nodejs" + "/";},
    ...
    //测试website 服务器目录
    targetDevelopServerPath:function(){return path.join(this.targetWebRootPath(),  "/");},
...
function copyReleaseFrameworkFiles(){
    ...
    var frameworkThemePath = config.workCopyFilesTemplatePath() + "/node-server/";
...
function initPathEnv(defines){
    ...
    if (currentPath.indexOf('autocoder')>-1){
        ...
        serverRoot = path.join(process.cwd(), config.sideName);
    }

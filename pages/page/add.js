...
class EditForm extends React.Component {
    ...
    handleSubmitUpdate(data) {
        ...
        model.add(data, function(response) {
            if (response && response.data) {
                console.log(data);
                router.push({pathname:'/page/list',query:{...that.props.query}});
            }
        })
    ...
    handleSaveAndEdit(childModuleName,data) {
        ...
        model.add(data, function(response) {
                if (response && response.data) {
                    ...
                    let params = {...that.props.query,pageId:response.data.id,fromModule:'page'};
    ...
    handleAssociationEdit(associationModule,referm,data) {
        ...
        model.add(data, function(response) {
            if (response && response.data) {
                console.log(response.data);
                let params = {...that.props.query,moduleName:"page",moduleId:response.data.id,associationName:associationModule,referModule:referm};
...
render()
{
    ...
    return (
            ...
            <Form  onSubmit={this.handleSubmit.bind(this)}>
                ...
                <Form.Item >
                    <XList  onEdit ={that.onSaveAndEdit.bind(that,'pageinterface')} refer ="pageinterface" mapField="" byId='-1'  title="专属接口" />
                </Form.Item>
                
                <Form.Item >
                    <XList  onEdit ={that.onAssociationEdit.bind(that,'pagepageinterface','')} refer ="pagepageinterface" mapField="pageId" byId='-1'  title="用到公用接口" />
                </Form.Item>

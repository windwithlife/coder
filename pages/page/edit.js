...
class EditForm extends React.Component {
    ...
    componentWillMount(){
        ...
        var that = this;
        console.log("edit id:=" + this.props.query.pageid);
        model.queryById(this.props.query.pageId,function(response) {
    ...
    handleSaveAndEdit(childModuleName,data) {
        ...
        let that = this;
        let params = {...that.props.query,fromModule:'page'};
    ...
    onAssociationEdit(aName,referm,e){
        e.preventDefault();
        var that = this;
        var mId = this.props.query.pageId;
        let params = {...that.props.query,moduleName:"page",moduleId:mId,associationName:aName,referModule:referm};
        router.push({pathname:'/page/association',query:params});
    }
    handleSubmitUpdate(data) {
        ...
        model.update(data, function(response) {
            if (response && response.data) {
                ...
                router.push({pathname:'/page/list',query:params});
            }
        })
   ...
   // let selectIndex = listItems.sex? listItems.sex:-1;
    ...
    return (
            ...
            <Form  onSubmit={this.handleSubmit.bind(this)}>
                    ...
                    <Form.Item >
                        <XList  onEdit ={that.onSaveAndEdit.bind(that,"pageinterface")} refer ="pageinterface" mapField="" byId={that.props.query.pageId}  title="专属接口" />
                        </Form.Item>
                
                    <Form.Item >
                        <XList  onEdit ={that.onAssociationEdit.bind(that,'pagepageinterface','pageinterface')} refer ="pagepageinterface" mapField="pageId" byId={that.props.query.pageId}  title="用到公用接口" />
                    </Form.Item>

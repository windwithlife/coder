...
class EditForm extends React.Component {
    ...
    componentWillMount(){
        ...
        var that = this;
        console.log("edit id:=" + this.props.query.pxmoduleid);
        model.queryById(this.props.query.pxmoduleId,function(response) {
    ...
    handleSaveAndEdit(childModuleName,data) {
        ...
        let that = this;
        let params = {...that.props.query,fromModule:'pxmodule'};
    ...
    onAssociationEdit(aName,referm,e){
        e.preventDefault();
        var that = this;
        var mId = this.props.query.pxmoduleId;
        let params = {...that.props.query,moduleName:"pxmodule",moduleId:mId,associationName:aName,referModule:referm};
        router.push({pathname:'/pxmodule/association',query:params});
    }
    handleSubmitUpdate(data) {
        ...
        model.update(data, function(response) {
            if (response && response.data) {
                ...
                router.push({pathname:'/pxmodule/list',query:params});
            }
        })
   ...
   // let selectIndex = listItems.sex? listItems.sex:-1;
    ...
    return (
            ...
            <Form  onSubmit={this.handleSubmit.bind(this)}>
                    ...
                    <Form.Item >
                        <XList  onEdit ={that.onAssociationEdit.bind(that,'','pxpage')} refer ="" mapField="pxmoduleId" byId={that.props.query.pxmoduleId}  title="表字段" />
                    </Form.Item>

                
                    <Form.Item >
                        <XList  onEdit ={that.onAssociationEdit.bind(that,'pxmodulepxtable','pxtable')} refer ="pxmodulepxtable" mapField="pxmoduleId" byId={that.props.query.pxmoduleId}  title="所用表" />
                    </Form.Item>

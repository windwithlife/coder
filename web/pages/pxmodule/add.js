...
class EditForm extends React.Component {
    ...
    handleSubmitUpdate(data) {
        ...
        model.add(data, function(response) {
            if (response && response.data) {
                console.log(data);
                router.push({pathname:'/pxmodule/list',query:{...that.props.query}});
            }
        })
    ...
    handleSaveAndEdit(childModuleName,data) {
        ...
        model.add(data, function(response) {
                if (response && response.data) {
                    ...
                    let params = {...that.props.query,pxmoduleId:response.data.id,fromModule:'pxmodule'};
    ...
    handleAssociationEdit(associationModule,referm,data) {
        ...
        model.add(data, function(response) {
            if (response && response.data) {
                console.log(response.data);
                let params = {...that.props.query,moduleName:"pxmodule",moduleId:response.data.id,associationName:associationModule,referModule:referm};
...
render()
{
    ...
    return (
            ...
            <Form  onSubmit={this.handleSubmit.bind(this)}>
                ...
                <Form.Item >
                    <XList  onEdit ={that.onAssociationEdit.bind(that,'','')} refer ="" mapField="pxmoduleId" byId='-1'  title="表字段" />
                </Form.Item>
                
                <Form.Item >
                    <XList  onEdit ={that.onAssociationEdit.bind(that,'pxmodulepxtable','')} refer ="pxmodulepxtable" mapField="pxmoduleId" byId='-1'  title="所用表" />
                </Form.Item>

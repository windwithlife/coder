...
class EditForm extends React.Component {
    ...
    handleSubmitUpdate(data) {
        ...
        model.add(data, function(response) {
            if (response && response.data) {
                console.log(data);
                router.push({pathname:'/pxpage/list',query:{...that.props.query}});
            }
        })
    ...
    handleSaveAndEdit(childModuleName,data) {
        ...
        model.add(data, function(response) {
                if (response && response.data) {
                    ...
                    let params = {...that.props.query,pxpageId:response.data.id,fromModule:'pxpage'};
    ...
    handleAssociationEdit(associationModule,referm,data) {
        ...
        model.add(data, function(response) {
            if (response && response.data) {
                console.log(response.data);
                let params = {...that.props.query,moduleName:"pxpage",moduleId:response.data.id,associationName:associationModule,referModule:referm};
...
render()
{
    ...
    return (
            ...
            <Form  onSubmit={this.handleSubmit.bind(this)}>
                ...
                <Form.Item >
                    <XList  onEdit ={that.onAssociationEdit.bind(that,'pxpagexinterface','')} refer ="pxpagexinterface" mapField="pxpageId" byId='-1'  title="用到公用接口" />
                </Form.Item>
